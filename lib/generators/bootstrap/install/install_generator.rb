...
module Bootstrap
  module Generators
    class InstallGenerator < ::Rails::Generators::Base
      ...
      argument :stylesheets_type, :type => :string, :default => 'scss', :banner => '*scss or static'
      ...
      def add_assets
        ...
        if File.exist?(css_manifest)
          ...
          if content.match(/require_tree\s+\.\s*$/)
            # Good enough - that'll include our bootstrap_and_overrides.css.scss
          else
      ...
      def cleanup_legacy
        ...
        if File.exist?('app/assets/stylesheets/bootstrap_override.css.scss')
          puts <<-EOM
          Warning:
            app/assets/stylesheets/bootstrap_override.css.scss exists
            It should be removed, as it has been superceded by app/assets/stylesheets/bootstrap_and_overrides.css.scss
          EOM
        end
      end
    ...
    private
      def use_scss?
        (defined?(Scss) && (stylesheets_type!='static') ) || (stylesheets_type=='scss')
      end
